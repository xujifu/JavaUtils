...
public class InvateCodeAlgorithms {
    ...
    private static final char[] BASE_CHAR = new char[]{
        'D', 'X', 'G', 'W', 'F', 'K',
        'T', 'R', '6', '9', '7', 'S',
        'P', 'A', '4', 'V', 'Y', 'B',
        'U', 'J', 'E', 'N', '8', '2',
    ...
    public String simplecode(long uid){
        long tuid = uid;
        StringBuffer sb = new StringBuffer();
        while(tuid > 0){
            sb.append(r[(int)(tuid % r.length)]);
            tuid = tuid / r.length;
        }
        if(sb.length() < 6) {
            Random r = new Random();
            for (int i = 0; i < 6 - sb.length(); i++) {
                int num = r.nextInt(this.r.length);
                sb.append(this.r[num]);
            }
        }
        return sb.toString();
    }
...
//        System.out.println(tree.size() - code.size());
        System.out.println(a.toSerialCode(594552576));
        System.out.println(a.simplecode(594552577));
//        for(long i = 0; i < 25000000l; i ++){
//            String str = a.toSerialCode(i);
//            tree.add(str);
//            code.add(str);
//        }
//        System.out.println(tree.size() - code.size());
